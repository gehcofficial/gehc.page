...
async function main() {
  ...
  let ok = 0;
  // Run in groups of 5 — close+reopen connection every 5 queries
  for (let i = 0; i < ALL.length; i += 5) {
    ...
    for (const [uid, name] of slice) {
      ...
      await conn.query(
        ...
        [JSON.stringify(gd.top5), JSON.stringify(gd.scores), uid]
      );
      ok++;
    }
    await conn.end();
    if (ok % 10 === 0) process.stdout.write('  ' + ok + '/' + ALL.length + '\n');
    await sleep(1000); // 1s cooldown between connections
  }

  const vconn = await mysql.createConnection({
    host: url.hostname, port: parseInt(url.port) || 4000,
    user: url.username, password: url.password,
    database: url.pathname.replace('/',''),
    ssl: { rejectUnauthorized: true },
  });
  const [count] = await vconn.query('SELECT COUNT(*) as c FROM users WHERE gifts_top5 IS NOT NULL');
