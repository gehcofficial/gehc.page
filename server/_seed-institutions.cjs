...
(async () => {
  ...
  for (const row of CATALOG) {
    await conn.query(
      `INSERT INTO institutions (id, slug, name, kind, city) VALUES ('${esc(row.id)}','${esc(row.slug)}','${esc(row.name)}','${esc(row.kind)}',${row.city ? `'${esc(row.city)}'` : 'NULL'})
       ON DUPLICATE KEY UPDATE name = VALUES(name), kind = VALUES(kind), city = VALUES(city)`
    );
  }
  const [[{ c }]] = await conn.query('SELECT COUNT(*) AS c FROM institutions');
  console.log('institutions', c);
  await conn.end();
})().catch((e) => {
