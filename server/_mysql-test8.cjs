...
async function main() {
  ...
  console.log('Connected. Testing', ALL.length, 'users with raw SQL...');

  let ok = 0;
  for (const [uid, name] of ALL) {
    const gd = genGifts(name);
    const top5 = esc(JSON.stringify(gd.top5));
    const scores = esc(JSON.stringify(gd.scores));
    const sql = `UPDATE users SET gifts_top5 = '${top5}', gifts_scores = '${scores}', is_beyonders = 1 WHERE id = '${uid}'`;
    const s = Date.now();
    const [r] = await conn.query(sql);
    ok++;
    console.log('  ' + ok + '/' + ALL.length + ' ' + uid + ': ' + (Date.now()-s) + 'ms');
  }
