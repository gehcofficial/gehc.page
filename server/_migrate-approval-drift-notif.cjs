/**
 * Idempotent: add APPROVAL_ITEM + DRIVE_DRIFT to notifications.type enum.
 */
...
async function main() {
  ...
  try {
    ...
    if (type.includes('APPROVAL_ITEM') && type.includes('DRIVE_DRIFT')) {
      console.log('notifications.type already has APPROVAL_ITEM + DRIVE_DRIFT — skip');
      return;
    }
    const list = ENUM_VALUES.map((v) => `'${v}'`).join(',');
    await conn.query(`ALTER TABLE notifications MODIFY COLUMN type ENUM(${list}) NOT NULL`);
    console.log('notifications.type updated with APPROVAL_ITEM + DRIVE_DRIFT');
  } finally {
    await conn.end();
  }
}
