/**
 * Idempotent: add EVENT_ARCHIVED to notifications.type enum.
 */
...
async function main() {
  ...
  try {
    ...
    if (type.includes('EVENT_ARCHIVED')) {
      console.log('notifications.type already has EVENT_ARCHIVED — skip');
      return;
    }
    const list = ENUM_VALUES.map((v) => `'${v}'`).join(',');
    await conn.query(`ALTER TABLE notifications MODIFY COLUMN type ENUM(${list}) NOT NULL`);
    console.log('notifications.type updated with EVENT_ARCHIVED');
  } finally {
    await conn.end();
  }
}
