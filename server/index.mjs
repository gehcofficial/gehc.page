...
app.post('/api/migrate/events', requireRole('SUPERADMIN'), wrap(async (req, res) => {
  ...
  const ddl = [
    ...
    "CREATE TABLE IF NOT EXISTS `internal_warta` (`id` VARCHAR(64) NOT NULL,`title` VARCHAR(200) NOT NULL,`summary` TEXT NULL,`body` MEDIUMTEXT NULL,`category` VARCHAR(30) NOT NULL DEFAULT 'UMUM',`share_user_id` VARCHAR(64) NULL,`share_note` VARCHAR(190) NULL,`attachments` JSON NULL,`link` VARCHAR(500) NULL,`deadline` DATE NULL,`status` VARCHAR(20) NOT NULL DEFAULT 'DRAFT',`is_pinned` BOOLEAN NOT NULL DEFAULT false,`view_count` INT NOT NULL DEFAULT 0,`created_by_id` VARCHAR(64) NULL,`published_at` DATETIME(3) NULL,`created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),`updated_at` DATETIME(3) NOT NULL, INDEX `internal_warta_status_published_idx`(`status`, `published_at`), INDEX `internal_warta_category_idx`(`category`), PRIMARY KEY (`id`)) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;",
