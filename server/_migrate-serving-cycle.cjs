    ...
    if (!tbl.length) {
      await conn.query(`
        CREATE TABLE serving_assignments (
          ...
          responsible_group_id VARCHAR(64) NOT NULL,
          ...
          CONSTRAINT fk_serv_assign_event FOREIGN KEY (event_id) REFERENCES \`EventProgram\`(id) ON DELETE SET NULL,
          CONSTRAINT fk_serv_assign_resp FOREIGN KEY (responsible_group_id) REFERENCES \`groups\`(id),
          CONSTRAINT fk_serv_assign_host FOREIGN KEY (host_group_id) REFERENCES \`groups\`(id)
