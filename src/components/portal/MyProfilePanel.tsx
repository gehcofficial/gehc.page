...
import { navigatePortal, roleToNamespace } from '../../lib/portal-routes';

export type ProfileSectionId = 'contact' | 'life' | 'gifts' | 'recreational' | 'emergency';

function MyBirthdayBanner({ name, age }: { name?: string | null; age?: number | null }) {
  ...
  const text = (caption || 'Selamat ulang tahun, {nama}! Tuhan Yesus memberkati di usia {umur} tahun. 🎉')
  ...
  return (
    <div className="rounded-[32px] overflow-hidden border border-pink-200 bg-gradient-to-br from-pink-50 via-white to-amber-50 shadow-sm">
      {photoUrl && <img src={photoUrl} alt="Ucapan ulang tahun" className="w-full max-h-52 object-cover" />}
      <div className="p-6 sm:p-8 text-center">
        <p className="text-2xl">🎉</p>
        <h2 className="text-xl font-black text-[#1B1B1B] mt-1">Selamat Ulang Tahun, {first}!</h2>
        <p className="text-xs text-[#5C5850] mt-2 leading-relaxed whitespace-pre-wrap">{text}</p>
...
export const MyProfilePanel: React.FC<{
  ...
  const { addToast, authUser, refreshAuthUser, currentRole, isMentee } = useApp();
  ...
  const isMyBirthdayToday = (() => {
    const raw = form.birthDate || user?.birthDate;
    if (!raw) return false;
    const d = new Date(raw);
    if (Number.isNaN(d.getTime())) return false;
    // Bandingkan bulan/tanggal dalam WIB.
    const nowWib = new Date(Date.now() + 7 * 60 * 60 * 1000);
    const mm = String(d.getUTCMonth() + 1).padStart(2, '0');
    const dd = String(d.getUTCDate()).padStart(2, '0');
    const todayMmDd = nowWib.toISOString().slice(5, 10);
    if (`${mm}-${dd}` === todayMmDd) return true;
    return mm === '02' && dd === '29' && todayMmDd === '02-28';
  })();

  return (
    <div className="space-y-5 max-w-2xl">
      {isMyBirthdayToday && <MyBirthdayBanner name={user?.name} age={age} />}
        ...
        {isMentee && authUser?.onboardingStatus !== 'WAITING_POOL' && (
          <div className="mt-4 rounded-[24px] border border-[#D9D7D0]/50 bg-white p-4 space-y-2">
            ...
            <p className="text-[10px] text-[#8C8880]">
              Tulis draf di panel Kesaksian. Marturia mengkurasi; Komisi menerbitkan ke landing.
            </p>
            <button
              type="button"
              onClick={() => navigatePortal({ namespace: roleToNamespace(currentRole), page: 'kesaksian' })}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#181818] text-white text-[11px] font-bold"
            >
              Buka panel Kesaksian
