...
export const PortalDashboard: React.FC<{ onNavigate: (page: string) => void }> = ({ onNavigate }) => {
  ...
  const totalMembersCount = groups.reduce((acc, g) => acc + g.memberCount, 0);

  // Group Monitoring Compliance Rate
  const groupsWithRecentMonitoring = groups.map((grp) => {
  ...
  const compliancePercentage = Math.round(
    (groupsWithRecentMonitoring.filter((g) => g.hasRecentLog).length / groups.length) * 100
  );
  ...
  return (
    <div className="space-y-8 animate-fade-in">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-white/60">• Portal Administrator</span>
            </div>
            ...
            <p className="text-xs sm:text-sm text-white/70 max-w-xl">
              ...
              {isMentor && userAssignedGroupId && (
                <span> Anda terikat pada pendampingan Kelompok <strong>{groups.find((g) => g.id === userAssignedGroupId)?.name}</strong>.</span>
              )}
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            >
              ...
              <span>{isMentor ? 'Input Monitoring Kelompok' : 'Monitoring 10 Kelompok'}</span>
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div className="bg-white rounded-3xl p-6 border border-[#D9D7D0]/50 shadow-sm flex flex-col justify-between">
          <div className="flex items-center justify-between mb-4">
            <span className="text-xs font-bold text-[#8C8880] uppercase tracking-wider">Total Anggota</span>
          ...
          <div>
            ...
            <p className="text-[11px] text-[#8C8880] mt-1">Terbagi dalam 10 kelompok pemuda</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl p-6 border border-[#D9D7D0]/50 shadow-sm flex flex-col justify-between">
          ...
          <div>
            ...
            <p className="text-[11px] text-emerald-600 font-semibold mt-1">
              {groupsWithRecentMonitoring.filter((g) => g.hasRecentLog).length} dari 10 grup terdata
            </p>
          </div>
        </div>
      ...
      {/* 10 Small Groups Monitoring Status Matrix */}
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-[#D9D7D0]/30">
              {groupsWithRecentMonitoring.map((grp) => {
                const isMentorGroup = userAssignedGroupId === grp.id;

                return (
                  <tr
                    key={grp.id}
                    className={`hover:bg-[#FAF9F5] transition-colors ${
                      isMentorGroup ? 'bg-blue-50/40 font-semibold' : ''
                    }`}
                  >
                    ...
                    <td className="py-3.5 pr-2 text-right">
                      ...
                      >
                        {isMentor && isMentorGroup ? 'Isi Laporan' : 'Lihat'}
                      </button>
                    </td>
                  </tr>
                );
      ...
      {/* Quick Integration & RBAC Status Bar */}
            ...
            <div>
              ...
              <p className="text-xs text-[#8C8880]">
                {isSuperAdmin
                  ...
                  : isMentor
                  ? 'Akses khusus monitoring kelompok binaan'
                  : 'Akses melihat profil dan konten publik'}
              </p>
            </div>
          </div>
